		...
		case "them": {
			...
			try {
				...
				if (usersDAO.kiemtra(request.getParameter("username"))
						...
						| !request.getParameter("repass").equals(request.getParameter("pass")) | !matcheremail.matches()
						| request.getParameter("username").length() < 5 | request.getParameter("username").length() > 15
						| !matcherpass.matches() | !matcherdienthoai.matches()) {
					...
					if (!matcherpass.matches()) {
						request.setAttribute("error5",
								"Mật khẩu chứa ít nhất 1 từ Hoa,1 từ thường,kèm 1 kí tự đặc biệt,Độ dài lớn hơn 8");
					}
					...
					users.setUser_code(uuid);
					{
						try {
							...
							String kichhoat = "http://localhost:8081/MuaBanNha/kichhoat.jsp?ID=" + uuid + "&TK="
									+ users.getUserEmail();
							SendMail.sendMail(request.getParameter("email"), "Hoan Tât Quá Trinh Đăng Ky", "Chào Ban,"
									+ users.getHovaten() + "\n"
					...
					if (users != null) {
						if (users.getUser_kichhoat() == 1) {
							...
							request.setAttribute("error2",
									users.getHovaten()
											+ "! ,Bạn Chưa Kích Hoạt Tài Khoản,1 Hộp Thư Đã Gửi Đến Địa Chỉ Email/Gmail "
											+ users.getUserEmail());
							url = "/login.jsp";
							break;
						}
